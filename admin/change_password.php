...
$hash = $rec['hash'] ?? ''; $msg = $err = '';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  require_csrf();
  $cur = $_POST['current'] ?? ''; $a = $_POST['a'] ?? ''; $b = $_POST['b'] ?? '';
  if ($a === '' || $a !== $b) $err = 'New passwords do not match.';
  elseif ($hash && !password_verify($cur, $hash)) $err = 'Current password is incorrect.';
  else {
    $newHash = password_hash($a, PASSWORD_DEFAULT);
    $rec = ['hash'=>$newHash, 'updated_at'=>date('c')];
    if (!@file_put_contents($PWD_JSON, json_encode($rec, JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES))) $err='Failed to write password file.';
...
<style>
  :root{--bg:#0e0f12;--panel:#0f1423;--line:#23283a;--fg:#eef2ff;--mut:#a6aec2}
  *{box-sizing:border-box} html,body{height:100%} body{margin:0;background:var(--bg);color:var(--fg);font-family:system-ui,Segoe UI,Inter,Arial}
  ...
  h1{margin:0 0 10px} label{display:block;margin:12px 0 6px;color:#c8cbe0}
  ...
  .muted{color:#9aa2b8}.ok{color:#b7f39a;margin-top:10px}.err{color:#ff9a9a;margin-top:10px} a{color:#cbd5ff}
</style></head><body>
<div class="wrap"><h1>Change Admin Password</h1><div class="panel">
  <form method="post" action="">
    <?= csrf_input() ?>
    <?php if($hash): ?>
      <label for="current">Current password</label><input type="password" id="current" name="current" required>
    <?php else: ?><div class="muted">No existing password found; you can set one now.</div><?php endif; ?>
    <label for="a">New password</label><input type="password" id="a" name="a" required>
    <label for="b">Confirm new password</label><input type="password" id="b" name="b" required>
