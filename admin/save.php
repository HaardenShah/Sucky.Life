<?php
/**
 * admin/save.php — Persist an egg + handle uploads
 ...
 * - Image pipeline: generates responsive WebP variants (-640/-1024/-1600) when GD is available
 * - Video: generates poster JPG via ffmpeg (if present)
 * - Audio: loudness normalization via ffmpeg (if present)
...
require __DIR__ . '/util.php'; // for slugify(), list_eggs(), load_egg(), etc.
...
function make_responsive_webp_variants($imageWebPath) {
  ...
  foreach ($sizes as $s) {
    $ratio = $h / $w;
    $nw = $s;
    $nh = (int)round($s * $ratio);
    $dst = imagecreatetruecolor($nw, $nh);

    // Preserve alpha for PNG input
    ...
    @imagewebp($dst, $webpAbs, 80);
    imagedestroy($dst);

    if (is_file($webpAbs)) {
      $out[] = web_path($webpAbs);
    }
  }
  imagedestroy($src);
  return $out;
...
function has_ffmpeg() {
  $o = @shell_exec('which ffmpeg 2>/dev/null');
  return !empty($o);
}
...
$variants = [];
if ($image_url) {
  $variants = make_responsive_webp_variants($image_url);
}

$poster = '';
if ($video_url) {
  $poster = ffmpeg_poster($video_url) ?: '';
}

if ($audio_url) {
  $norm = ffmpeg_loudnorm($audio_url);
  if ($norm) $audio_url = $norm;
}
