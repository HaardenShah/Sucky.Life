...
// Logout clears the visitor session
if (isset($_GET['logout'])) {
  unset($_SESSION['visitor_ok']);
  header('Location: /gate.php'); exit;
}

// If gate is off, just bounce home
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($GATE_HASH && password_verify($pw, $GATE_HASH)) {
    ...
    // Optional: longer visitor session (comment out if you don’t want it)
    // session_set_cookie_params(['lifetime'=>60*60*8]); // 8 hours next page load
...
<style>
  ...
  *{box-sizing:border-box} html,body{height:100%} body{margin:0; background:var(--bg); color:var(--fg); font-family:system-ui,Segoe UI,Inter,Arial; display:grid; place-items:center}
  ...
  input[type=password]{flex:1; padding:14px 14px; border-radius:12px; border:1px solid var(--line); background:#0b0f1a; color:var(--fg); font-size:16px; outline:none}
  ...
  .meta{margin-top:12px; font-size:12px; color:var(--mut)}
  .small{margin-top:14px; display:flex; justify-content:space-between}
  a{color:#cbd5ff}
</style>
</head>
<body>
  <form class="card" method="post" action="">
    ...
    <div class="small">
      <div class="meta">&copy; <?= date('Y') ?> <?= htmlspecialchars($SITE_DOMAIN ?: $SITE_NAME) ?></div>
      <div><a href="/gate.php?logout=1">Log out</a></div>
    </div>
  </form>
</body>
</html>