 ...
 * - Optional visitor gate (/gate.php)
 * - View token for egg iframes (prevents direct linking)
 * - Screech audio (Mute shows ONLY while playing)
 * - Tears only while playing & unmuted + cursor repel
 * - Hotspots from eggs/list.php
 * - Smooth modal (iframe) animations
 * - Visual Editor bridge (?from=editor posts vw/vh to parent)
 */
...
/* Visitor gate */
if (!empty($GATE_ON) && $GATE_ON === true && empty($_SESSION['visitor_ok'])) {
  $ret = urlencode($_SERVER['REQUEST_URI'] ?? '/');
  header("Location: /gate.php?return={$ret}");
  exit;
}

/* View token (for egg iframe access) */
...
<head>
  ...
  <style>
    :root{ --yellow:#ffcc00; --bg:#0e0f12; --fg:#f8f8f8; --dim:#b6b6b6; --bezier:cubic-bezier(.22,.61,.36,1) }
    ...
    .lip-text{ font-size:clamp(16px,2.2vw,22px); color:#b6b6b6; font-style:italic; text-align:center; cursor:pointer; user-select:none; transition: color .2s ease, transform .15s ease, text-shadow .2s ease; }
    ...
    .egg-spot{ position:fixed; width:28px; height:28px; border-radius:50%; background:rgba(255,255,255,.06); outline:1px dashed rgba(255,255,255,.12); transform:translate(-50%,-50%) scale(.96); cursor:help; transition:transform .28s var(--bezier), outline-color .25s ease, background .25s ease, box-shadow .25s ease; box-shadow:0 6px 20px rgba(0,0,0,.35); z-index:4 }
    ...
    .modal .dialog{ position:relative; width:min(92vw,900px); height:min(80vh,580px); border-radius:16px; overflow:hidden; border:1px solid rgba(255,255,255,.2); box-shadow:0 30px 80px rgba(0,0,0,.6); background:#0b0b0b; transform:translateY(12px) scale(.98); opacity:0; transition:transform .34s var(--bezier), opacity .3s ease }
    ...
    @media (prefers-reduced-motion: reduce){ .lip-text.playing{ animation:none } .tear{ animation-duration:4s } }
  </style>
</head>
<body>
  ...
  <main>
    <div>
      ...
      <div class="hint">(Mobile browsers block autoplay with sound. Tap once and we&rsquo;ll do the rest.)</div>
    </div>
  </main>
  ...
  <script>
    ...
    function spawnTear(){ const t=document.createElement('div'); t.className='tear'; t.style.left=(Math.random()*100)+'vw'; t.style.top='-20px'; const dur=1.8+Math.random()*2.2; t.style.animationDuration=dur+'s'; t.style.opacity=(0.7+Math.random()*0.3).toFixed(2); tears.appendChild(t); setTimeout(()=>t.remove(), dur*1000); }
    ...
    const audio = document.getElementById('screech'), btnPlay = document.getElementById('btnPlay'), btnMute = document.getElementById('btnMute');
    function haptics(){ try{ if(navigator.vibrate) navigator.vibrate([20,40,20]); }catch(_){} }
    function setUI(playing){
      if(playing){ btnPlay.innerHTML='Stop the suffering'; btnPlay.setAttribute('aria-label','Stop'); btnMute.hidden=false; btnMute.classList.toggle('muted',audio.muted); btnMute.querySelector('.label').textContent = audio.muted ? 'Unmute' : 'Mute'; }
      else{ btnPlay.innerHTML='Make it extra sucky <span class="pulse"></span>'; btnPlay.setAttribute('aria-label','Play'); btnMute.hidden=true; }
      updateLipText();
    }
    ...
    btnPlay.addEventListener('click', async ()=>{ try{ haptics(); if(audio.paused) await playAudio(); else stopAudio(); }catch{} });
    ...
    function repel(e){ const els=document.getElementsByClassName('tear'); for(const el of els){ const r=el.getBoundingClientRect(); const cx=r.left+r.width/2, cy=r.top+r.height/2; const dx=cx-e.clientX, dy=cy-e.clientY; const dist=Math.hypot(dx,dy); if(dist<repelRadius){ const s=(1-dist/repelRadius); const nx=dx/(dist||1), ny=dy/(dist||1); const px=Math.max(Math.min(nx*maxPush*s*1.4,maxPush),-maxPush); const py=Math.max(Math.min(ny*maxPush*s*1.4,maxPush),-maxPush); el.style.setProperty('--pushX',px.toFixed(1)+'px'); el.style.setProperty('--pushY',py.toFixed(1)+'px'); } }
      clearTimeout(decayTimer); decayTimer=setTimeout(()=>{ for(const el of document.getElementsByClassName('tear')){ el.style.setProperty('--pushX','0px'); el.style.setProperty('--pushY','0px'); } },140);
    }
    ...
    const eggModal=document.getElementById('eggModal'), eggFrame=document.getElementById('eggFrame'), modalBackdrop=document.getElementById('modalBackdrop'), closeModalBtn=document.getElementById('closeModal'); let modalOpen=false;
    function openEgg(src){
      if(!src || modalOpen) return; modalOpen=true;
      const sep = src.includes('?') ? '&' : '?';
      eggFrame.src = src + sep + 'vt=<?= htmlspecialchars($EGG_VIEW_TOKEN) ?>';
      eggModal.classList.remove('closing'); eggModal.classList.add('show'); eggModal.setAttribute('aria-hidden','false'); document.body.style.overflow='hidden'; haptics();
    }
    function hideEgg(){
      if(!modalOpen) return; modalOpen=false; eggModal.classList.add('closing'); eggModal.classList.remove('show'); eggModal.setAttribute('aria-hidden','true');
    ...
    modalBackdrop.addEventListener('click', hideEgg); closeModalBtn.addEventListener('click', hideEgg); document.addEventListener('keydown', e=>{ if(e.key==='Escape' && eggModal.classList.contains('show')) hideEgg(); });

    fetch('eggs/list.php', {cache:'no-store'}).then(r=>r.json()).then(items=>{
      (items||[]).filter(i=>typeof i.pos_left==='number' && typeof i.pos_top==='number' && !i.draft).forEach(addHotspot);
    }).catch(()=>{});
    function addHotspot(item){
      const spot=document.createElement('div'); spot.className='egg-spot'; spot.style.left=item.pos_left+'vw'; spot.style.top=item.pos_top+'vh'; spot.title=item.slug; spot.dataset.slug=item.slug; spot.addEventListener('click', ()=> openEgg('eggs/egg.php?slug='+encodeURIComponent(item.slug))); document.body.appendChild(spot);
    ...
    ['play','pause','ended','volumechange'].forEach(ev=> audio.addEventListener(ev, updateLipText)); updateLipText();

    document.addEventListener('visibilitychange', ()=>{ if(document.hidden) stopTears(); else updateTears(); });

    (function(){ const fromEditor = new URLSearchParams(location.search).get('from')==='editor'; if(!fromEditor) return;
      function ping(x,y){ const d=document.createElement('div'); d.style.cssText='position:fixed;width:14px;height:14px;border-radius:50%;background:rgba(255,204,0,.95);box-shadow:0 6px 20px rgba(0,0,0,.45);transform:translate(-50%,-50%);z-index:61;left:'+x+'px;top:'+y+'px'; document.body.appendChild(d); setTimeout(()=>d.remove(),450); }
      function onPlace(ev){ const vw=(ev.clientX/window.innerWidth)*100; const vh=(ev.clientY/window.innerHeight)*100; try{ window.parent.postMessage({type:'egg-editor-click',vw,vh},'*'); }catch(_){} ping(ev.clientX,ev.clientY); }
      document.addEventListener('click', function(e){ if(e.target.closest('.modal')||e.target.closest('button')) return; onPlace(e); e.preventDefault(); e.stopPropagation(); }, true);
    })();
  </script>
</body>
</html>
